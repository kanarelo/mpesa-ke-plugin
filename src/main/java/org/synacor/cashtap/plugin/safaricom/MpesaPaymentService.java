...
import org.synacor.cashtap.models.Contact;
...
import org.synacor.cashtap.models.LogMessage;
import org.synacor.cashtap.models.OutgoingPayment;
import org.synacor.cashtap.models.Target;
...
//> INSTANCE PROPERTIES
	...
	public void sendAmountToPaybillAccount(final String businessName, final String businessNo, final String accountNo, final BigDecimal amount) {
		...
		User client;
		if (clientDao.getUserByPhoneNumber(businessNo) == null){
			client = new User(businessName,"",businessNo);
			client.setActive(false);
			clientDao.saveUser(client);
		} else {
			client = clientDao.getUserByPhoneNumber(businessNo);
		}
					...
					cService.doSynchronized(new SynchronizedWorkflow<Object>() {
						public Object run() throws SMSLibDeviceException,
							...
							final StkResponse enterBusinessNumberResponse = cService.stkRequest(
									enterBusinessNumberPrompt.getRequest(), outgoingPayment.getUser().getPhoneNumber());
			...
			public void run() {
				...
				try {
					...
					if (account != null){
						...
						if (clientDao.getUserByPhoneNumber(getPhoneNumber(message))!=null) {
							//save the incoming payment in generic account
							account = accountDao.getGenericAccountsByUserId(clientDao.getUserByPhoneNumber(getPhoneNumber(message)).getId());
							...
							final User client = new User(firstName,otherName,getPhoneNumber(message));
							clientDao.saveUser(client);

							//Start Save the User as a contact to the core project
							...
							reportPaymentFromNewUser(createIncomingPayment(account,null, message));
					    }
