...
import org.synacor.cashtap.service.ui.PaymentServiceUiActionHandler;
					...
					cService.doSynchronized(new SynchronizedWorkflow<Object>() {
						public Object run() throws SMSLibDeviceException, IOException {
							...
							final StkResponse enterPhoneNumberResponse = cService.stkRequest(
									...
									outgoingPayment.getUser().getPhoneNumber());
							try {							
...
//>END - OUTGOING PAYMENT REGION
	...
	Account getAccount(FrontlineMessage message) {
		User client = clientDao
				.getUserByPhoneNumber(getPhoneNumber(message));
		if (client != null) {
			List<Account> activeNonGenericAccountsByUserId = accountDao
					.getActiveNonGenericAccountsByUserId(client.getId());
			if (!activeNonGenericAccountsByUserId.isEmpty()) {
				return activeNonGenericAccountsByUserId.get(0);
			} else {			
				Account genericAccount = accountDao.getGenericAccountsByUserId(client.getId()); 
				if(genericAccount != null) {
					...
					return accountDao.getGenericAccountsByUserId(client.getId());
				}
			}
		}
		return null;
